...
namespace PrendaSAL.Catalogos
{
    ...
    public partial class ClienteVIPForm : Form
    {
        //PARA MANTENER UNA INSTANCIA UNICA DE LA CLASE//
        private static ClienteVIPForm frmInstance = null;

        public static ClienteVIPForm Instance()
        {
            if (((frmInstance == null) || (frmInstance.IsDisposed == true)))
            {
                frmInstance = new ClienteVIPForm();
            }
        ...
        private void ClienteVIPForm_Load(object sender, EventArgs e)
        {
        ...
        {
            REGLAS = Reglas.ConvertToReglas(dbPrendasal.getReglas(eNIVEL.VIP));
            if (REGLAS != null)
            {
                TasaMensual_VIP.Value = (decimal)REGLAS.TASA_MENSUAL;
                PlazoContrato_VIP.Value = REGLAS.PLAZO_CONTRATO;
                PlazoVencido_VIP.Value = REGLAS.PLAZO_VENCIDO;
                DescCompras_VIP.Value = (decimal)REGLAS.DESC_COMPRA;
                DescInteres_VIP.Value = (decimal)REGLAS.DESC_INTERES;
                PuntosDolar_VIP.Value = (decimal)REGLAS.PUNTOS_DOLAR;
                PuntosValor_VIP.Value = REGLAS.PUNTOS_VALOR;
            }
        ...
        private void cargarTarjetaSelected()
        {
            if (SELECTED != null)
            {
               ...
               // CLIENTE = SELECTED.CLIENTE;
                ...
                TasaMensual_VIP.Value = (decimal)SELECTED.TASA_MENSUAL;
                PlazoContrato_VIP.Value = SELECTED.PLAZO_CONTRATO;
                PlazoVencido_VIP.Value = SELECTED.PLAZO_VENCIDO;
                DescCompras_VIP.Value = (decimal)SELECTED.DESC_COMPRA;
                DescInteres_VIP.Value = (decimal)SELECTED.DESC_INTERES;
                PuntosDolar_VIP.Value = (decimal)SELECTED.PUNTOS_DOLAR;
                PuntosValor_VIP.Value = SELECTED.PUNTOS_VALOR;
        ...
        public bool validarTarjeta()
        {
            ...
            try
            {
                ...
                {
                    OK = false;
                    MessageBox.Show("Seleccione Cliente VIP", "VALIDACION DE DATOS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return OK;
                }
            ...
            {
                OK = false;
                MessageBox.Show("ERROR AL VALIDAR TARJETA VIP", "ERROR VALIDAR DATOS", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return OK;
        }
        ...
        private Tarjeta buildTARJETA()
        {
            ...
            t.TASA_MENSUAL = TasaMensual_VIP.Value;
            t.PLAZO_CONTRATO = (int)PlazoContrato_VIP.Value;
            t.PLAZO_VENCIDO = (int)PlazoVencido_VIP.Value;
            t.DESC_COMPRA = DescCompras_VIP.Value;
            t.DESC_INTERES = DescInteres_VIP.Value;
            t.PUNTOS_DOLAR = (int)PuntosDolar_VIP.Value;
            t.PUNTOS_VALOR = PuntosValor_VIP.Value;
        ...
        private void ELIMINAR(object sender, EventArgs e)
        {
            if (SELECTED != null)
            {
                ...
                DialogResult eliminar = MessageBox.Show("¿Está seguro que desea eliminar la Tarjeta VIP # " + SELECTED.NUMTARJETA.ToString() + " EXPEDIDA EL:" + SELECTED.FECHA_EXP.Date.ToString("dd/MM/yyyy") + " ?", "ELIMINAR TARJETA", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        public void buscarTarjeta(string numero)
        {
            try
            {
                //SELECTED = dbTarjeta.findByNumTarjetaVIP(numero);
                if (SELECTED != null)
                {
                    cargarTarjetaSelected();
                }
        ...
        {
            string numTAR = Controles.InputBox("NUMERO DE TARJETA :", "AUTORIZACION");
            if (numTAR != "")
            {
                //SELECTED = dbTarjeta.findByNumTarjetaVIP(numTAR);
                if (SELECTED != null)
                {
                    cargarTarjetaSelected();
